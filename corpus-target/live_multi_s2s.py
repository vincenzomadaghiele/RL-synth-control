...
if __name__ == "__main__":
    ...
    corpus_scaler_benjolin_sth = joblib.load(f'{log_dir_benjolin_to_granular}/tgt_synth_scaler.pkl')
    synth_scaler_benjolin_sth = joblib.load(f'{log_dir_benjolin_to_granular}/src_synth_scaler.pkl')
    ...
    env_s2s = gym.make(
        training_parameters_benjolin_to_granular["environment"], features=training_parameters_benjolin_to_granular["features"],
        src_synth_type=training_parameters_benjolin_to_granular["src_synth_type"], corpus_path=training_parameters_benjolin_to_granular["corpus_path"],
        sample_rate = training_parameters_benjolin_to_granular["sample_rate"],FFT_window_size = training_parameters_benjolin_to_granular["FFT_window_size"],
        hop_size = training_parameters_benjolin_to_granular["hop_size"],N_steps_memory=training_parameters_benjolin_to_granular["N_steps_memory"],
        N_reward_memory=training_parameters_benjolin_to_granular["N_reward_memory"],max_episode_duration=training_parameters_benjolin_to_granular["max_episode_duration"],
        step_size=training_parameters_benjolin_to_granular["step_size"],rewards = training_parameters_benjolin_to_granular["rewards"],
        normalization_mode = training_parameters_benjolin_to_granular["normalization_mode"],episode_mode = training_parameters_benjolin_to_granular["episode_mode"],
        synths_info_dir='./corpus-target/02_synthesizers',save_folder=log_dir_benjolin_to_granular,train=False,render_mode=None
    )
    ...
    # LOAD MODEL S2S
    AGENT_TYPE = training_parameters_benjolin_to_granular["AGENT_TYPE"]
    MODEL_DIR = f'{log_dir_benjolin_to_granular}/best'
    if AGENT_TYPE == 'PPO':
        model_benjolin = PPO.load(MODEL_DIR, env=env_benjolin)
    elif AGENT_TYPE == 'A2C':
        model_benjolin = A2C.load(MODEL_DIR, env=env_benjolin)
    ...
    input_buffer_s2s = Buffer(1, window_size*3)
    synth_buffer_s2s = Buffer(1, window_size*3)
    ...
    graph.add_node(FeedbackBufferWriter(input_buffer_s2s, granular_sth, min_feedback_writer_delay))
    graph.add_node(FeedbackBufferWriter(synth_buffer_s2s, benjolin_sth, min_feedback_writer_delay))
    ...
    input_sound_s2s = input_buffer_s2s.data[0,:]
    benjolin_sound_s2s = synth_buffer_s2s.data[0,:]
    ...
    prev_audioin_features_s2s = computeFeatures(input_sound_s2s, training_parameters_theremin["features"], sr, window_size, hop_size)
    prev_audioin_features_s2s = corpus_scaler_theremin.transform(np.array(prev_audioin_features_s2s))
    previous_src_features_s2s = computeFeatures(benjolin_sound_s2s, training_parameters_theremin["features"], sr, window_size, hop_size)
    prev_audioin_features_s2s = synth_scaler_theremin.transform(np.array(previous_src_features_s2s))
